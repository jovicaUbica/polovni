...
(async () => {
    ...
    try {
        ...
        console.error('Kritična greška u scraperu:', globalError);
        
        // Pokušavamo da pošaljemo kritičnu grešku na Telegram pre nego što pukne
        try {
            const bot = new TelegramBot(TELEGRAM_TOKEN, { polling: false });
            const errMsg = `🚨 *Kritična greška u scraperu!*\n\nSkripta se srušila pri pokretanju:\n\`\`\`\n${globalError.message || globalError}\n\`\`\``;
            await bot.sendMessage(CHAT_ID, errMsg, { parse_mode: 'Markdown' });
            console.log('Greška uspešno poslata na Telegram.');
        } catch (tgErr) {
            console.error('Nije uspelo slanje greške na Telegram:', tgErr.message);
        }
